<?php
/**
 * Relevanssi WP-CLI command.
 */
class Relevanssi_Command extends WP_CLI_Command {
    ...
    function index( $args, $assoc_args ) {
    	$post_id = null;
        if (isset($assoc_args["post"])) $post_id = $assoc_args["post"];

        $limit = null;
        if (isset($assoc_args["limit"])) $limit = $assoc_args["limit"];

        $extend = false;
        if (isset($assoc_args["extend"])) $extend = $assoc_args["extend"];

        $debug = false;
        if (isset($assoc_args["index_debug"])) $debug = $assoc_args["index_debug"];

		$target = null;
        if (isset($assoc_args["target"])) $target = $assoc_args["target"];
     ...
     *
     */
    function truncate_index( $args, $assoc_args ) {
     ...
     *
     */
    function add_stopword( $args, $assoc_args ) {
		if (is_array($args)) {
     ...
     *
     */
    function remove_stopword( $args, $assoc_args ) {
    	$verbose = false;
		if (is_array($args)) {
			foreach ($args as $stopword) {
     ...
     *
     */
    function reset_log( $args, $assoc_args ) {
    	$verbose = false;
     ...
     *
     */
    function common( $args, $assoc_args ) {
    	$wp_cli = true;

        $limit = 25;
        if (isset($assoc_args["limit"]) && is_numeric($assoc_args["limit"])) $limit = $assoc_args["limit"];
     ...
     * 
     */
    function index_pdfs( $args, $assoc_args ) {
        global $wpdb;
    	...
    	$pdf_attachments = relevanssi_get_posts_with_pdfs();
...
WP_CLI::add_command( 'relevanssi', 'Relevanssi_Command' );

function relevanssi_generate_progress_bar($title, $count) {
  $progress = null;
  if ( defined( 'WP_CLI' ) && WP_CLI ) $progress = WP_CLI\Utils\make_progress_bar( $title, $count );
  return $progress;
}
